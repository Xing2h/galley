...
| 3.13. v0.1.1 内置 Python · 零配置 attach | 🚧 Phase 0+1 已落地，Phase 3/6/7 未启动 | issue #1（lauchiwa Win uv 用户）+ Mac alpha.1 prod build dogfood + GA 群多次反馈"Python 路径配置烦"汇聚到一个结构性结论：v0.1 把 Python 候选路径硬编码到 Tauri shell capability allowlist 注定填不完。**结论**：Galley 自 v0.1.1 起在 release 里内嵌 CPython 3.11.15（PBS install_only_stripped）+ GA core deps（requests/bs4/bottle/simple-websocket-server），用户零 Python 配置可用。**关键发现**：GA core deps 测出来 wheel 总大小 0.93MB（之前 hedge 的"30s pip install + 网络依赖"完全不必要）。Bundle 每架构 ~80MB 解开 / ~28MB 压缩进 .dmg。[完整 devlog](docs/devlog/2026-05-18-v0.1.1-bundled-python.md)。**未签名策略 codified**：JC 拍板「Galley 目前阶段不考虑签名，未签名发布，作为个人开源项目问题不大」——写进 CLAUDE.md 防止再次纠结。**Phase 0 POC ✅**：JC Intel Mac 跑 `pnpm tauri build --target x86_64-apple-darwin` → 装 → 右键打开 → attach GA → 发消息 → `pgrep -fl workbench_bridge` 显示 `/Applications/Galley.app/Contents/Resources/python/bin/python3` spawn，GA 正常响应。**Phase 1 ✅**：feature/v0.1.1-bundled-python 4 commits 落地——(1) `scripts/bundle-python.sh` PBS downloader + GA deps stage + .gitignore × 2；(2) `tauri.conf.json` resources mapping + `capabilities/default.json` 两 alias（mac/linux + win）；(3) `bridge.ts` PROD&&!useExternalPython 双门控 + gaConfig 加 useExternalPython 字段 + alpha.2→v0.1.1 配置自动迁移（默认 false 让升级用户享用 bundle）；(4) 顺手修 alpha.1 起 Settings → About 显示 "v0.1.0" 撒谎 bug（demo fixture hardcode，改 hydrateFromDB 调 Tauri getVersion()）。**Phase 3 / 6 / 7 未启动**：Onboarding StepHealth Python row 改"Galley 内置 Python ✓" + Settings → Runtime advanced toggle "使用外部 Python" + `python-missing-anthropic` tutorial 退役（Phase 3）；CI release.yml matrix 加 macos-15 x86_64-apple-darwin cross-compile row + 每 job 跑 bundle-python.sh 对应 arch + PBS tarball actions/cache（Phase 6）；bump 三处 version 到 v0.1.1-alpha.1 + tag push + dogfood Mac arm64 + Win + 回复 issue #1（Phase 7） |
