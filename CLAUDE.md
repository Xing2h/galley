...
| 8. B3: useAppStore 拆 slice + 改订阅 Rust event · [playbook](./docs/refactor/B3-store-slice.md) | 🟡 M1-M3 ✅，M4 待 fresh session | M1-M3 落地（2026-05-19）：**M1 design paperwork**（[slice mapping](./docs/refactor/b3-slice-mapping.md) + [ADR](./docs/refactor/b3-slice-adr.md) + [Rust emit catalogue](./docs/refactor/b3-rust-emit-catalogue.md)）/ **M2 uiStore**（display state，net -61 行）/ **M3 runtimeStore**（M3a LLM concerns + M3b bridge lifecycle，net -1000 LOC from useAppStore，[devlog](./docs/devlog/2026-05-19-b3-m3-complete.md)）。沿途穿插：B3 prereq relaxation 把 1 周日历仪式改成事件驱动双层 gate / [perf baseline P1-P4 实测](./docs/refactor/perf-baseline.md) / **4 个 B2 latent bug 修**（CLI 提问 GUI 渲染 `4e7a6e6` / LLM 持久化 `9c36f42` / picker flash `d6a096f` / picker spawning click `317e816`）。**M4 sessionsStore (~550 行)** 必须先动 Rust 加 18 个 session/project CRUD trait method（[G5](./docs/refactor/B3-store-slice.md#running-notes--gotchas) 警示）独立 commit 不混 frontend。剩 M5 messagesStore (~600 行，最复杂 — streaming + approval + ask_user + auto-scroll 互动) + M6 prefsStore (~250 行) + 收尾。`useAppStore.ts` 从 ~2700 → 1700 行已 ship，最终目标 ≤ 200 行或彻底删除 |
