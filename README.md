...
**关于 GA 版本**：Workbench 跑你本地 GenericAgent 仓库的当前 HEAD，**升级节奏由你掌握**。Settings → Runtime 显示当前 commit 和我们测试过的 baseline。我们对 baseline 之后的 upstream commits 做过接口兼容审计，但 GA 不提供 API 稳定性保证——你 `git pull` 后如果出现兼容问题，bridge 启动体检会报错。
