...
**两 header 共通视觉规约**
- 状态控件统一视觉语法：文字 badge 统一 28px 高度、6px 圆角、12px 字号、border / hover / press 节奏；icon-only 状态统一 28px 方形按钮、Radix tooltip、focus ring。`warning` / `error` / `success` / `neutral` 只表达状态，不给某个功能单独造身份视觉。
...
- Hover / focus 图标打开 quick-fill popover，显示最多 5 条手动置顶的
  prompt。每行显示标题 + 单行正文预览；点击只预填 Composer，不自动发送。
- Popover 从右侧工具按钮向上弹出并 `align=end`，避免挡住 Composer 中央。
- 点击图标本身打开提示词库 dialog。Dialog 顶部不显示可见副标题；主体为
  卡片平铺，先显示最多 5 条置顶 prompt，再显示其他 prompt。整张卡片点击即
  预填 Composer，不自动发送；hover / focus 卡片时显示管理按钮。
- 卡片管理动作：预设只读，可置顶 / 取消置顶 / 调整置顶顺序 / 复制为自定义；
  自定义可新增、编辑、删除、置顶、调整置顶顺序。置顶图标在置顶卡片上常显。
- 数据存在 GUI prefs `saved_prompts_v1`，只保存用户自定义 prompt 和
  `pinnedIds`；内置 preset 不写入 prefs，随 UI 语言本地化。
