...
- Hover 图标打开 quick-fill popover，显示最多 5 条手动置顶的
...
- 点击图标本身打开提示词库 dialog。Dialog 顶部不显示可见副标题；尺寸约
  `920x680`，读作 Settings / Earlier / Archived 同族的工作台，而不是小确认窗。
  主体为工作台式 `bg-app` 画布 + 卡片平铺，先显示最多 5 条置顶 prompt，再显示
  其他 prompt。整张卡片点击即预填 Composer，不自动发送；hover 卡片时显示查看 /
  管理按钮。
- 卡片只承载摘要：标题、类型、置顶状态和 4 行正文预览。需要看完整 prompt 时，
  hover 点“查看”进入同一 dialog 内的完整阅读页；阅读页提供返回、填入输入框、
  置顶 / 取消置顶，以及预设复制为自定义 / 自定义编辑。
- 卡片管理动作：预设只读，可置顶 / 取消置顶 / 调整置顶顺序 / 复制为自定义；
  自定义可新增、编辑、删除、置顶、调整置顶顺序，也可在未置顶列表里手动上移 /
  下移调整自定义展示顺序。置顶图标在置顶卡片上常显。
