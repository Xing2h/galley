...
- 右：分成两个清晰 group，最后才是 Windows window controls（不属于工具簇）：
...
- TopBar 状态控件使用同一套视觉语法：文字 badge 统一 28px 高度、6px 圆角、12px 字号、border / hover / press 节奏；icon-only 状态统一 28px 方形按钮、Radix tooltip、focus ring。`warning` / `error` / `success` / `neutral` 只表达状态，不给某个功能单独造身份视觉。
...
- **不显示**：runtime（继续在 Sidebar 顶部，不进入 TopBar）/ Stop（在 Composer Submit 位置）/ Context Window / 价格
