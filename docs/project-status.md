...
- Git tag / GitHub Release: `v0.2.11` is the current published stable release.
- Agent API schema: `schemaVersion: 1`
- Release tier: stable patch; default update channel points at `v0.2.11`.
...
`v0.2.11` ships conversation font-size controls, Chinese typography polish,
sidebar click-through fixes, and process-lifecycle hardening for Mac heat /
process pile-up reports. It adds parent-death self-exit to
`workbench_bridge.py`, passes the owning Galley Core PID into runner
subprocesses, and prevents duplicate GUI/Core startup from creating a second
background process set when an existing instance already owns the local socket.
Product shape, Agent API schema, database schema, GA baseline, and
update-channel policy stay unchanged.

## Current Release State

`v0.2.11` is published and promoted as the live stable patch. The default
`updates/stable/latest.json` channel points at `v0.2.11`, with the legacy
...
1. Dogfood the app-update path from an installed older Galley build to
   `v0.2.11`.
2. Keep watching process-lifecycle reports around background mode, true Quit,
   duplicate startup, and runner cleanup.
3. On Windows, continue smoke coverage for duplicate startup / named-pipe
...
| Release path | v0.2.11 stable patch is published and promoted on the stable update channel | [release / update SOP](./release-update-sop.md) |
