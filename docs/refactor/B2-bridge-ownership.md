...
```
Cursor:   T7.1  (M1-M6 done; next: dogfood + acceptance + devlog + tag)
Status:   🚧 M1-M6 COMPLETE · M7 pending
Started:  2026-05-19
Last touch: 2026-05-19 — M6 ship same session: agent-api.md gained socket transport,
            session.send + session.watch schemas, Origin type, error tag catalogue
Predecessor: B1 ✅ (commits 4ee23e3 → 41cdeb5)
Successor:   B3 (useAppStore 拆 slice + 改订阅 Rust event)
Duration:    3 周估计（D16-D30），实际单 session 推到 M6 (~6h cumulative)
```
