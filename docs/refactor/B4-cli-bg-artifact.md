...
```
Cursor:   M2 prereq gate · tray spike scaffold + dogfood window
Status:   ✅ M1 COMPLETE (4 commits: dd4f6cf · 3cfb8de · 8f1f4b0 · f461ba0) · 157/157 cargo + typecheck + lint clean · M2 gated on tray spike
Started:  2026-05-20 (paperwork)
Last touch: 2026-05-20 PM — M1.4 closeout commit shipped (`f461ba0`)：agent-api.md +296 lines covering §5.8-§5.18 11 commands + §1 stable discriminant list + §3 exit code 5 row + §6 socket envelope + §8 planned (removed shipped items, added v0.6+ session kill / project archive reversible / session watch --from / llm warmup) + status banner updated。cli/tests/m1_writes.rs new file 17 integration tests (8 session.* + 5 project.* + 4 llm.*) — covers exit-4 no-Core path for socket writes, full happy/empty/shape-error for direct-SQLite reads (project list + llm list), and clap-surface validation (session new full flag set, session move with/without --to)。**M1 整体 metrics**: 4 commits 跨 1 day calendar, sub-plan estimate 1-2 weeks could slip to 3-4。140 → 157 cargo test。所有 PRD §11.1 write commands reachable from CLI。`session.new` SQLite tx atomicity (O1) covered via core/tests/db_writes_test.rs tx_* trio。M1.1 `dd4f6cf` + M1.2 `3cfb8de` + M1.3 `8f1f4b0` shipped earlier same day. **Next**: M2 sub-plan + tray spike scaffold + actually-run (gate-blocking, sub-plan §6 dogfood window 也开始计时)。
