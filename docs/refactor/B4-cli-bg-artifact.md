...
```
Cursor:   T1.11 · agent-api.md §5.7-§5.17 + integration tests (M1.4 commit — docs + tests, M1 closeout)
Status:   📋 Playbook ready · M1.3 project + llm 5 cmd shipped 2026-05-20 · M1.4 next
Started:  2026-05-20 (paperwork)
Last touch: 2026-05-20 PM — M1.3 project+llm commit shipped (`8f1f4b0`)：3 socket handler (project.create mint id server-side + emit project-created-external / project.delete snapshot child sessions BEFORE delete + emit project-deleted-external with detachedSessions count + ids / llm.set 复用 resolve_llm_name + best-effort manager.send_command(SetLlm) + emit session-updated-external) + 5 CLI subcommand (project create/list/delete + llm list/set；list 走 SQLite 直读 bypass socket) + 2 GUI listener (project-created/deleted-external) + 3 sessionsStore action (applyExternalSessionUpdated 扩 selectedLlmIndex/displayName patch + applyExternalProjectCreated race-guarded prepend + applyExternalProjectDeleted FK SET NULL mirror)。620 LOC across 4 files。Tests: 140/140 cargo + typecheck/lint clean。M1.1 `dd4f6cf` + M1.2 `3cfb8de` shipped earlier same day. **M1.4 remaining**: agent-api.md §5.7-§5.17 11 段 schema + §3 exit code 5 row + §1 stable error discriminants + 15-22 integration tests (cli/tests/m1_*.rs)，sub-plan §3 T1.11+T1.12 spec。
...
- [ ] **T1.7** `agent-api.md` 增量写入 — 每个命令 ship 时同 commit 加 schema 段 (M1.4 next, sub-plan §3 T1.11)
- [ ] **T1.8** CLI integration tests — 各命令 happy + error path (M1.4 next, sub-plan §3 T1.12)
