...
```
Cursor:   T3.1  (M3 runtimeStore 抽离 — 待 fresh session 开干)
Status:   🟢 M2 ✅ (informally dogfood-validated 2026-05-19 after attach_pet fix) · M3 deferred to fresh session per JC scope call
Started:  2026-05-19
Last touch: 2026-05-19 — M2 + B2 attach_pet fix shipped。M3 一次推完风险评估过大（拆 SessionRuntime + 9 action + cross-store onClose + 9 module-level 删除 = 多文件 800+ 行改动），JC 决定下 session fresh state 重开 M3。详 N5
