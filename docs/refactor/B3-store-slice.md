...
```
Cursor:   T6.17 dogfood  (M6 impl ✅ ship 完，等 JC dogfood + M7 acceptance)
Status:   🟢 M2 ✅ · M3 ✅ · M4 ✅ · M5 ✅ · M6 ✅ (sub-plan + implementation)
Started:  2026-05-19
Last touch: 2026-05-20 — **M6 实施完**（破 sub-plan「M5 dogfood + fresh session」启动门 per JC explicit `commit，然后继续推进`）。Single commit M6 9 文件 +505/-580 LOC = net -75：删 useAppStore.ts -465 行 + 新 prefs.ts 378 行 + 新 lib/hydrate.ts 127 行 + App.tsx 14 hook swap (useAppStore → usePrefsStore + seedMockSessions → useSessionsStore) + Onboarding.tsx 1 hook swap + runtime.ts import + dispatchIPCEvent caller + helper rename `readGAConfigFromPrefs` + sessions.ts dynamic → static import + ipc-handlers.ts 删 store param + `__store` DevTools shortcut → `__prefs`。**勘误**: sub-plan §1.1 估 prefsStore ~280 行实际 378 行（含完整 docstring 保留）+ lib/hydrate.ts ~120 行实际 127 行（接近预算）。**沿途清干净**：runtime.ts/sessions.ts/messages.ts/bridge.ts/ipc-handlers.ts 全部 stale `useAppStore` doc comment 重写为新位置；TRANSITIONAL labels 全删；V3 grep `useAppStore` / `TRANSITIONAL` 全 0。126/126 Rust tests + TS typecheck + lint 0 warning + cargo check clean。**M5 + M6 dogfood 都留下次 session 跑** — V5 auto-scroll + V6 turnIndexOffset PK + V7 mirror correctness (M5) + V1-V10 + 7 cluster scenarios (M6)。M7 启动条件：M5 + M6 dogfood 通过。详 N15-N16。
